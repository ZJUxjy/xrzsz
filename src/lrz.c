  ...
  while ((c = getopt_long(argc, argv, "a+bB:cCDeEhm:M:OprRqs:St:uUvw:XZy",
    ...
    case 'U':
      if (!under_rsh)
        ...
        DO_SYSLOG(
            (LOG_INFO, "--unrestrict option used under restricted shell"));
        error(1, 0,
...
static void usage1(int exitcode) { usage(exitcode, NULL); }
...
static int wcreceive(int argc, char **argp) {
  ...
  if (protocol != ZM_XMODEM || argc == 0) {
...
#ifdef ENABLE_SYSLOG
      if (enable_syslog)
        lsyslog(LOG_ERR, "%s/%s: cannot open: %m", shortname, protname());
#endif
      return ERROR;
    }
    if (wcrx(&zi) == ERROR) {
    ...
    if (Verbose > 1
      ...
      bps = (zi.bytes_received - zi.bytes_skipped) / d;
      if (Verbose) {
        vstringf(_("\rBytes received: %7ld   BPS:%-6ld                \r\n"),
                 (long)zi.bytes_received, bps);
      }
#ifdef ENABLE_SYSLOG
      if (enable_syslog)
        lsyslog(LOG_INFO, "%s/%s: %ld Bytes, %ld BPS", shortname, protname(),
...
static int wcrx(struct zm_fileinfo *zi) {
  ...
  for (;;) {
    ...
    purgeline(0); /* Do read next time ... */
    sectcurr =
        wcgetsec(&Blklen, secbuf, (unsigned int)((sectnum & 0177) ? 50 : 130));
    report(sectcurr);
...
static int procheader(char *name, struct zm_fileinfo *zi) {
  ...
  if (zconv != ZCRESUM && !Rxclob && (zmanag & ZF1_ZMMASK) != ZF1_ZMCLOB &&
    ...
    if (zmanag == ZF1_ZMNEW || zmanag == ZF1_ZMNEWL) {
      if (-1 == fstat(fileno(fout), &sta)) {
        ...
        DO_SYSLOG(
            (LOG_ERR, "cannot fstat open file %s: %s", name, strerror(e)));
        return ERROR;
      }
      if (zmanag == ZF1_ZMNEW) {
  ...
  if (Topipe > 0) {
    ...
    if (!fout) {
      ...
      DO_SYSLOG((LOG_ERR, "%s: cannot open: %s", protname(), strerror(e)));
      return ERROR;
    }
  }
buffer_it:
...
static int rzfile(struct zm_fileinfo *zi) {
    ...
    if (anker) {
      ...
      for (akt = anker, last = NULL; akt; last = akt ? akt : last, akt = next) {
        if (akt->pos == zi->bytes_received) {
          ...
          vfile("using saved out-of-sync-paket %lx, len %ld", akt->pos,
                akt->len);
          goto nxthdr;
        }
        next = akt->next;
  ...
  skip_oosb:
...
#endif
      if (--n < 0) {
        DO_SYSLOG_FNAME((LOG_INFO, "%s/%s: error: zgethdr returned %s",
                         shortname, protname(),
                         c == ZNAK ? "ZNAK" : "TIMEOUT"));
        vfile("rzfile: zgethdr returned %d", c);
        return ERROR;
    ...
    case ZSKIP:
      ...
      closeit(zi);
      DO_SYSLOG_FNAME(
          (LOG_INFO, "%s/%s: error: sender skipped", shortname, protname()));
    ...
    case ZDATA:
      if (rclhdr(Rxhdr) != (long)zi->bytes_received) {
        ...
        if (--n < 0) {
          ...
          DO_SYSLOG_FNAME(
              (LOG_INFO, "%s/%s: error: out of sync", shortname, protname()));
          return ERROR;
        }
#if defined(SAVE_OOSB)
        ...
        case GOTCRCQ:
          if (pos > zi->bytes_received) {
            ...
            if (neu && neu->data) {
              ...
              lsyslog(LOG_ERR, "saving out-of-sync-block %lx, len %lu", pos,
    ...
    moredata:
      if ((Verbose > 1 || min_bps || stop_time) &&
          ...
          if (low_bps) {
            if (last_bps < min_bps) {
              if (now - low_bps >= min_bps_time) {
                /* too bad */
                vfile(_("rzfile: bps rate %ld below min %ld"), last_bps,
                      min_bps);
        ...
        if (stop_time && now >= stop_time) {
          ...
          DO_SYSLOG_FNAME(
              (LOG_INFO, "%s/%s: reached stop time", shortname, protname()));
          return ERROR;
        }
      ...
      case ZCAN:
        ...
        DO_SYSLOG_FNAME(
            (LOG_INFO, "%s/%s: zrdata returned ZCAN", shortname, protname()));
        return ERROR;
      case ERROR: /* CRC error */
        ...
        if (--n < 0) {
          vfile("rzfile: zgethdr returned %d", c);
          DO_SYSLOG_FNAME((LOG_INFO, "%s/%s: zrdata returned ERROR", shortname,
                           protname()));
          return ERROR;
        }
        zmputs(Attn);
