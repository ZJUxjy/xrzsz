...
static void printtm(it) struct tm *it;
{
  printf("%02d/%02d/%04d %02d:%02d:%02d (%s) yday:%03d dst:%d gmtoffset:%ld",
         it->tm_mon + 1, it->tm_mday, it->tm_year + 1900, it->tm_hour,
         it->tm_min, it->tm_sec, it->tm_zone, it->tm_yday, it->tm_isdst,
         it->tm_gmtoff);
}
#endif

static time_t dist_tm(t1, t2) struct tm *t1;
struct tm *t2;
{
  time_t distance = 0;
...
void do_normalization(tmptr) struct tm *tmptr;
{
...
#define normalize(foo, x, y, bar)                                              \
  ...
  normalize(tm_mday, 1,
            __mon_lengths[__isleap(tmptr->tm_year + 1900)]
                         [((tmptr->tm_mon < 0) ? (12 + (tmptr->tm_mon % 12))
                                               : (tmptr->tm_mon % 12))],
            tm_mon);
...
time_t __mktime_internal(timeptr, producer) struct tm *timeptr;
...
static struct tm *localtime_r(t, tp) const time_t *t;
struct tm *tp;
{
