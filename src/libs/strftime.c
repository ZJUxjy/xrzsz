...
static struct tm *gmtime_r(t, tp) const time_t *t;
struct tm *tp;
{
...
register const struct tm *tp;
{
  ...
  const char *const ampm = _NL_CURRENT(LC_TIME, hour12 > 11 ? PM_STR : AM_STR);
    ...
    case 'z': /* GNU extension.  */
...

      {
        ...
        if (lt == (time_t)-1) {
          ...
          if ((ltm.tm_sec ^ tm.tm_sec) | (ltm.tm_min ^ tm.tm_min) |
